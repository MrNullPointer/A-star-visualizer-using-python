...
def algorithm(draw, grid, start, end):
    ...
    }  # distance from start to curr node
    g_score[start] = 0
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            ):  # Ex initially all g scores are set to infinity, so if new g score is less update it and set the parent node to current node
...
def make_grid(rows, width):
    ...
    node_size = (
        width // rows
    )  # will give width of each cube (or the dostance b/w each row)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]:  # if left mouse button is pressed
                ...
                if (
                    not start and node != end
                ):  # if this is a mouse click, start is not set, set start as curr pos of mouse
                    start = node
                    start.make_start()
            ...
            if event.type == pygame.KEYDOWN:
                if (
                    event.key == pygame.K_SPACE and start and end
                ):  # to start we need an end and start
                    for row in grid:
                        for node in row:
